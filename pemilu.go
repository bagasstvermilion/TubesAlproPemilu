...
func menuUtama() int {
	/*Mengembalikan nilai pilihan dari menu utama*/
	var pilihan int
	for {
		fmt.Println("")
...
func menuKPU() int {
	/*Mengembalikan nilai pilihan dari menu KPU*/
    var pilihan int
    for {
        fmt.Println("")
...
func setVotingThreshold() {
	/*I.S : Terdefinisi votingThresold dari variabel global sebagai input
	  F.S : Ambang batas suara telah disesuaikan
	*/
...
func menuPeriksaData() int {
	/*Mengembalikan nilai pilihan dari menu periksa data*/
	var pilihan int
	for {
		fmt.Println("")
...
func menuPemilih() int {
	/*Mengembalikan nilai pilihan dari menu pemilih*/
	var pilihan int
	for {
		fmt.Println("")
...
func menuEdit() int {
	/*Mengembalikan nilai pilihan dari menu edit*/
	var pilihan int
	for {
		fmt.Println("")
...
func setVotingTime() {
	/*I.S. : terdefinisi startYear, startMonth, startDay, startHour, startMinute sebagai input untuk menentukan waktu mulai pemilihan
			 terdefinisi endYear, endMonth, endDay, endHour, endMinute sebagai input untuk menentukan waktu berakhirnya pemilihan
	  F.S. : Rentang waktu pemilihan telah ditentukan
	*/
...
func isVotingTime() bool {
	/*Mengembalikan nilai boolean untuk menentukan apakah sudah waktunya pemilihan atau belum*/
...
func editDataPartai(A *tabPartai, nCalon int) bool {
	/*Mengubah nama partai dan mengembalikan nilai boolean true jika proses berhasil dijalankan atau false jika gagal*/
	var i int
...
func editDataPemilih (A *tabPemilih, nPemilih int, idPemilih string) bool {
	/*Mengubah NIK pemilih dan mengembalikan nilai boolean true jika proses berhasil dijalankan atau false jika gagal */
	var i int
	var gantiId string
...
func tambahCalon(A *tabCalon, B tabPartai, nPartai int, n *int) {
	/*I.S. : Terdefinisi A sebagai array dari calon, B sebagai array dari partai, nPartai sebagai jumlah data dari partai dan n sebagai jumlah data
	         Terdefinisi variabel calon dari struct tCalon, sebagai tempat menaruh data sementara dari input
	  F.S. : Data calon akan ditambahkan berupa nama, partai dan boolean wni
	*/
...
func hapusDataCalon(A *tabCalon, nCalon *int, namaCalon string) bool {
	/*Menghapus data calon dan mengembalikan nilai boolean true jika proses berhasil atau false jika gagal*/
	var i, j int
...
func hapusDataPemilih(A *tabPemilih, nPemilih *int, idPemilih string) bool {
	/*Menghapus data pemilih dan mengembalikan boolean true jika proses berhasil atau false jika gagal*/
	var i, j int
...
func hapusDataPartai(A *tabPartai, nPartai *int, namaPartai string) bool {
	/*Menghapus data partai dan mengembalikan boolean true jika proses berhasil atau false jika gagal*/
	var i, j int
...
func tambahPemilih(A *tabPemilih, nPemilih *int) {
	/*I.S. : Terdefinisi A sebagai array dari pemilih, dan nPemilih sebagai jumlah data dari pemilih
	  F.S. : Jumlah pemilih berhasil ditambahkan lewat NIK
	*/
	var i, tambahan int
	fmt.Println("")
...
func periksaNikPemilih(A tabPemilih, nPemilih int) {
	/*I.S. : Terdefinisi A sebagai array dari pemilih dan nPemilih sebagai jumlah data dari pemilih
	  F.S. : Menampilkan NIK pemilih yang sudah terdaftar dan boolean dari status pemilihan, jika true maka sudah memilih atau false jika belum
	*/
	var i, j, minIdx int
	for i = 0; i < nPemilih; i++ {
		minIdx = i
...
func tambahPartai(A *tabPartai, nPartai *int) {
	/*I.S. : Terdefinisi A sebagai array dari partai dan nPartai sebagai jumlah data dari partai
	  F.S. : Jumlah partai bertambah berupa nama partai
	*/
	var i, tambahan int
	fmt.Println("")
...
func insertionSortByName(A *tabCalon, n int) {
	/*I.S. : Terdefinisi A sebagai array dari calon dan n sebagai jumlah data, sorting menggunakan Insertion Sort
	  F.S. : Hasil akan terurut sesuai dari indeks nama dari A-Z 
	*/
	var i, j int
	var key tCalon
	for i = 1; i < n; i++ {
...
func cariCalonBerdasarkanPartai(A tabCalon, nCalon int, partai string) {
	/*I.S. : Terdefinisi A sebagai array dari calon, nCalon sebagai jumlah data dari calon dan partai sebagai input dari calon yang akan dicari
	  F.S. : Menampilkan calon dari partai yang dicari secara terurut berdasarkan nama
	*/
	var ditemukan bool
	var i, x int
...
func editDataCalon(A *tabCalon, n int, namaCalon string) bool {
	/*Merubah nama dari calon dan mengembalikan boolean true jika proses berhasil dilakukan atau false jika gagal*/
	var i int
...
func tampilkanHasil(A tabCalon, nCalon int) {
	/*I.S. : Terdefinisi A sebagai array dari calon dan nCalon sebagai jumlah data dari calon
	  F.S. : Menampilkan hasil dari seluruh calon yang diurutkan berdasarkan jumlah suara secara Descending dengan menggunakan Selection Sort
	*/
    var i, j, minIdx int
    for i = 0; i < nCalon; i++ {
        minIdx = i
...
func pilihCalon(A *tabPemilih, B *tabCalon, nPemilih int, nCalon int) {
	/*I.S. : Terdefinisi A sebagai array dari pemilih, B sebagai array dari calon, nPemilih sebagai jumlah data dari pemilih dan nCalon sebagai jumlah data dari calon
	  F.S. : Calon akan terpilih dan jumlah suara akan bertambah
	*/
...
func cariCalon(A tabCalon, nCalon int, namaCalon string) {
	/*I.S. : Terdefinisi A sebagai array dari calon, nCalon sebagai jumlah data dari calon, dan namaCalon sebagai input yang akan dicari nama calonnya
	  F.S. : Menampilkan hasil dari calon yang dicari berupa nama, partai, dan suara
	*/
	var i int
	fmt.Println("")
...
func pencarianPemilihBerdasarkanCalon(A tabPemilih, B tabCalon, nCalon int, namaCalon string) {
	/*I.S. : Terdefinisi A sebagai array pemilih, B sebagai array calon, nCalon sebagai jumlah data dari calon, dan namaCalon sebagai input yang akan akan dicari nama calonnya
	  F.S. : Menampilkan seluruh pemilih yang memilih calon yang divcari secara terurut berdasarkan NIK nya secara Ascending menggunakan Selection Sort
	*/
	var i, j, minIdx int
	for i = 0; i < nCalon; i++ {
		minIdx = i
